...
function renderReferences(items) {
  ...
  items.forEach((item) => {
    ...
    const pageText = item.page_end && item.page_end !== item.page
      ? `${item.page}-${item.page_end}`
      : `${item.page}`;
    const typeText = item.metadata?.type === "table" ? "表格" : "文本";
    wrapper.innerHTML = `
      <p class="reference-meta">${item.source_file} | 页码 ${pageText} | 类型 ${typeText} | 分数 ${item.score.toFixed(4)}</p>
