...
Zip bundles are extracted under ``raw_dir/`` and normalized so the adapter can
read a root-level ``content_list.json``. Self-hosted deployments that return a
JSON result body instead of a zip are also supported by extracting the embedded
content list and materializing it as ``content_list.json``.
"""
...
class MinerURawClient:
    ...
    async def _download_zip(
        ...
        """Download a MinerU result and materialize ``content_list.json``.

        Preferred path is the traditional zip bundle. Some self-hosted
        ``mineru-api`` deployments return JSON directly from
        ``GET /tasks/{task_id}/result``; in that case we extract the embedded
        content list and persist it at ``raw_dir/content_list.json`` so the rest
        of the adapter can remain unchanged.
        """
        ...
        try:
            ...
            self._write_json_result_bundle(resp, raw_dir, result_url)
            return
    ...
    def _write_json_result_bundle(
        self,
        resp: Any,
        raw_dir: Path,
        result_url: str,
    ) -> None:
        """Persist a JSON MinerU result as ``raw_dir/content_list.json``."""
        try:
            payload = resp.json()
        except Exception as exc:
            snippet = (resp.text or "")[:500] if hasattr(resp, "text") else ""
            raise RuntimeError(
                "MinerU result is neither a zip archive nor valid JSON "
                f"(url={result_url}, content_type={resp.headers.get('content-type', '')!r}, "
                f"snippet={snippet!r})"
            ) from exc
        ...
        if content_list is None:
            raise RuntimeError(
                "MinerU JSON result did not contain a usable content_list "
                f"(url={result_url}, top_level_keys={sorted(payload.keys()) if isinstance(payload, dict) else type(payload).__name__})"
            )
        ...
        logger.info(
            "[mineru_raw] materialized JSON result bundle at %s",
            raw_dir / CONTENT_LIST_FILENAME,
        )
