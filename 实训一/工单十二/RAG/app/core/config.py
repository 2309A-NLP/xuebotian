...
class Settings(BaseSettings):
    ...
    ragas_judge_embedding_model_name: str | None = Field(
        default=None,
        alias="RAGAS_JUDGE_EMBEDDING_MODEL_NAME",
    )
    ragas_judge_embedding_device: str | None = Field(
        default=None,
        alias="RAGAS_JUDGE_EMBEDDING_DEVICE",
    )
    ragas_judge_timeout: float | None = Field(default=None, alias="RAGAS_JUDGE_TIMEOUT")
    ragas_eval_timeout: int = Field(default=300, alias="RAGAS_EVAL_TIMEOUT")
    ragas_eval_max_workers: int = Field(default=2, alias="RAGAS_EVAL_MAX_WORKERS")
    ragas_eval_max_retries: int = Field(default=4, alias="RAGAS_EVAL_MAX_RETRIES")
    ragas_eval_max_wait: int = Field(default=30, alias="RAGAS_EVAL_MAX_WAIT")
    ragas_eval_batch_size: int | None = Field(default=None, alias="RAGAS_EVAL_BATCH_SIZE")
    ragas_eval_raise_exceptions: bool = Field(default=False, alias="RAGAS_EVAL_RAISE_EXCEPTIONS")
