...
class RagPipeline(
    ...
    def stream_answer(
        ...
        for token in self.llm_client.stream_chat(
            ...
            yield self._sse_event("token", {"delta": token})
        final_answer = "".join(final_answer_parts).strip()
        llm_elapsed = time.perf_counter() - llm_started

        total_elapsed = time.perf_counter() - started_at
        logger.info(
            "QA stream timing | embed=%.3fs search=%.3fs llm=%.3fs total=%.3fs hits=%s variants=%s question=%s",
            embed_elapsed,
            search_elapsed,
            llm_elapsed,
            total_elapsed,
