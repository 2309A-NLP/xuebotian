...
class RagRankingMixin:
    ...
    def _structure_bonus(
        ...
        *,
        ...
        # BM25检索时，文档标题包含问题中的公司名，加权
        org_phrases = self._org_phrases(question)
        if org_phrases:
            retrieval_mode = str(hit.metadata.get("retrieval_mode") or "")
            if retrieval_mode == "keyword_bm25" and document_title:
                if any(org in document_title for org in org_phrases):
                    bonus += 0.12  # BM25模式下文档标题含公司名，加显著加权
            else:
                # 非BM25模式，检查chunk全文
                chunk_text = f"{document_title} {heading} {hit.text or ''}"
                org_match_count = sum(1 for org in org_phrases if org in chunk_text)
                if org_match_count > 0:
                    bonus += min(org_match_count * 0.05, 0.15)
    ...
    def _table_bonus(
        ...
        *,
        ...
        # BM25检索时，文档标题包含问题中的公司名，加权
        org_phrases = self._org_phrases(question)
        doc_title = str(metadata.get("document_title") or "").strip()
        if org_phrases:
            retrieval_mode = str(metadata.get("retrieval_mode") or "")
            if retrieval_mode == "keyword_bm25" and doc_title:
                if any(org in doc_title for org in org_phrases):
                    bonus += 0.12  # BM25模式下文档标题含公司名，加显著加权
            else:
                # 非BM25模式，检查chunk全文
                chunk_text = f"{doc_title} {caption} {text}"
                org_match_count = sum(1 for org in org_phrases if org in chunk_text)
                if org_match_count > 0:
                    bonus += min(org_match_count * 0.05, 0.15)
